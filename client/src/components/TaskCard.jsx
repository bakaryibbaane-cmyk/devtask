    ...
    <Draggable draggableId={task._id} index={index}>
      {(provided, snapshot) => (
        <div
          ...
          className={`bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-3 transition-shadow ${
        ...
        >
          ...
          <div className="flex gap-2 justify-end">
            <button onClick={() => onEdit(task)} className="text-xs text-gray-500 hover:text-blue-600">
              ✏️ Modifier
            </button>
            <button onClick={() => onDelete(task._id)} className="text-xs text-gray-500 hover:text-red-600">
              🗑️ Supprimer
            </button>
          </div>
        </div>
